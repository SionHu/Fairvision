...
def parse(words):
    ...
    for group in __groups_re__.findall(words):
        ...
        group_multiplier = 1
        if group[1] in __groups__:
            group_multiplier = __groups__[group[1]]
        ...
        hundreds_match = __hundreds_re__.match(group[0])
        ...
        if hundreds_match is not None and hundreds_match.group(1) is not None:
            ...
            # running tally
            group_num = group_num + \
                        (__ones__[hundreds_match.group(1)] * 100)
        ...
        tn1_match = __tens_and_ones_re__.match(tens_and_ones)
        # if the pattern is matched, there is a 'tens' place value
        if tn1_match is not None:
            # add the tens
            group_num = group_num + __tens__[tn1_match.group(1)]
            # add the ones
            if tn1_match.group(2) is not None:
                group_num = group_num + __ones__[tn1_match.group(2)]
        else:
        # assume that the 'tens and ones' actually contained only the ones-
        # place values
            group_num = group_num + __ones__[tens_and_ones]
        # increment the total number by the current group number, times
        # its multiplier
        num = num + (group_num * group_multiplier)
